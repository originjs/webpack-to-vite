...
import {
  ...
  getGreaterVersion,
  processDependencies
} from '../src/generate/genePackageJson'
import fs from 'fs'
...
describe('genePackageJson', () => {
  ...
  test('generate normal package.json', () => {
    const testPackageJsonPath = path.resolve(
      'tests/testdata/package-json/package-normal.json'
    )
    ...
    const testPostcssConfigPath = path.resolve(
      'tests/testdata/package-json/postcss.config.js'
    )
  ...
  test('generate vue2 package.json', () => {
    const testPackageJsonPath = path.resolve(
      'tests/testdata/package-json/package-vue2.json'
    )
    const packageJsonPath = path.resolve('tests/out-package-json/package.json')
    fs.copyFileSync(testPackageJsonPath, packageJsonPath)
    genePackageJson(packageJsonPath, packageJsonPath)
  ...
  test('generate vue3 package.json', () => {
    const testPackageJsonPath = path.resolve(
      'tests/testdata/package-json/package-vue3.json'
    )
    const packageJsonPath = path.resolve('tests/out-package-json/package.json')
    fs.copyFileSync(testPackageJsonPath, packageJsonPath)
    genePackageJson(packageJsonPath, packageJsonPath)
...
test('processDependencies', () => {
  ...
  const resultA = processDependencies(
    originPackageJsonA.dependencies,
    packageJsonA.dependencies,
    packageJsonA.devDependencies
  )
  ...
  const resultB = processDependencies(
    originPackageJsonB.dependencies,
    packageJsonB.dependencies,
    packageJsonB.devDependencies
  )
  expect(resultB).toEqual({
    restDependencies: undefined,
    targetDependencies: {
      sass: '^1.1.1'
    }
  })
