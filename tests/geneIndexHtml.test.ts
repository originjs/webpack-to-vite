...
import {TransformationType} from "../src/ast-parse/transformations";
...
describe('generateWithVueCliPublicIndex', () => {
    test('generateWithVueCliPublicIndex form none ast parsing result', () => {
        const result = generateWithVueCliPublicIndex(null, ['src/main.js'], 'vue-cli')
    ...
    test('generateWithVueCliPublicIndex form webpack ast parsing result', () => {
        ...
        const astParsingResult: AstParsingResult = {
            ...
            transformationResult: {
                [TransformationType.indexHtmlTransformationWebpack]: [
                    {
                        fileInfo: {
                            path: '',
                            source: ''
                        ...
                        content: content,
                        type: TransformationType.indexHtmlTransformationVueCli
                    }
                ]
            }
        }
        const result = generateWithVueCliPublicIndex(astParsingResult, ['src/main.js'], 'webpack')
    ...
    test('generateWithVueCliPublicIndex form vue-cli ast parsing result', () => {
        ...
        const astParsingResult: AstParsingResult = {
            parsingResult: {},
            transformationResult: {
                [TransformationType.indexHtmlTransformationVueCli]: [
                    {
                        fileInfo: {
                            path: '',
                            source: ''
        ...
        const result = generateWithVueCliPublicIndex(astParsingResult, ['src/main.js'], 'vue-cli')
...
describe('getEntries', () => {
    ...
    test('getEntries from array', () => {
        const result = getEntries(path.resolve('tests/out-index-html'), ['./app1.js', './app2.js'])
...
describe('getDefaultEntries',  () => {
    test('getDefaultEntries from vue.config.js', async () => {
        ...
        const result = await getDefaultEntries(path.resolve('tests/out-index-html'), 'vue-cli')
